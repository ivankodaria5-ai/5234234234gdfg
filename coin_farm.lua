-- MM2 Farm Hub v7 | Tween flight + Death snap | Anti-detect
...
local CFG = {
    ...
    DashUrl      = "https://branches-cumulative-desktops-bearing.trycloudflare.com",
}
...
pcall(function()
    ...
    if rsEv then
        rsEv.OnClientEvent:Connect(function()
            -- If RoundStart fires within 20 sec of joining = we joined mid-round
            if tick() - joinedAt < 20 then
                roundActive  = true
                roundEndDone = false
                print("[Hub] Joined mid-round - skipping this round")
                return
            end
            roundActive  = true
            roundEndDone = false
